...
import type { TaskType } from '@/lib/study/types';
  ...
  <form className="mt-3 grid gap-3">
    ...
    <div className="grid grid-cols-2 gap-3">
      <label className="grid gap-1">
        ...
        <input
          ...
          value={dueValue}
          onChange={(e) =>
            updateTask(task.id, { dueDate: e.target.value ? e.target.value : null })
          }
          className="w-full rounded-md border border-[var(--mutedblack)] bg-[var(--background)] px-3 py-2 text-sm text-[var(--lightshadow)] outline-none"
        />
      </label>
    ...
    {task.type === "tick" || task.type === "supervision" ? (
      <SelectField
        ...
        value={(task as any).moduleId ?? ""}   // safe cast; narrowed below when you want stricter typing
        placeholder="Select module"
        onChange={(v) =>
          updateTask(task.id, { moduleId: v || null } as any)
        }
      >
        
        {modulesForCourse.map((m: any) => (
    ...
    {task.type === "supervision" ? (
      <div className="grid gap-3">
        <div className="grid grid-cols-2 gap-3">
          <SelectField
            label="Supervision #"
            value={String((task as any).svNum ?? "")}
            placeholder="Select number"
            onChange={(v) => updateTask(task.id, { svNum: Number(v) } as any)}
          >
            <option value="">-</option>
          ...
          <label className="grid gap-1">
            ...
            <input
              value={(task as any).supervisorId ?? ""}
              onChange={(e) => updateTask(task.id, { supervisorId: e.target.value } as any)}
              className="w-full rounded-md border border-[var(--mutedblack)] bg-[var(--background)] px-3 py-2 text-sm text-[var(--lightshadow)] outline-none"
        ...
        <label className="grid gap-1">
          ...
          <textarea
            value={(task as any).work ?? ""}
            onChange={(e) => updateTask(task.id, { work: e.target.value } as any)}
    ...
    <label className="grid gap-1">
      ...
      <div className="mt-6" />
            <Button onClick={() => {  if (!confirm('Delete this task? This cannot be undone.')) return;
    deleteTask(task.id);
    router.push('/tasks'); }}>Delete</Button>
